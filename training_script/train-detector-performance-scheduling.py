...
def process_data_item(data_item, dim, model_stride):
    ...
    XX, llp, pts = augment_sample(
        data_item[0], data_item[1].pts, dim)  # XX là (208,208,3)
    YY = labels2output_map(llp, pts, dim, model_stride)  # YY là (13,13,9)
...
if __name__ == '__main__':
    ...
    model, model_stride, xshape, yshape = load_network(
        args.model, dim)  # xshape là (208,208,3), yshape là (13,13,8)
    ...
    Ytrain = np.empty((batch_size, int(dim/model_stride),
                       int(dim/model_stride), 2*4+1))  # (batch,13,13,9)

    val_dir = './samples/val'
    Val_Data = load_Data(val_dir)
    Xval, Yval = process_Data(Val_Data)
    ...
    K.set_value(model.optimizer.learning_rate, base_lr)
