...
def conv_batch(_input, fsz, csz, activation='elu', padding='same', strides=(1, 1)):
    output = Conv2D(fsz, csz, activation='linear',
                    padding=padding, strides=strides)(_input)
...
def create_model_mobnet():
    ...
    backbone_layers = {'backbone_' +
                       layer.name: layer for layer in backbone.layers}
...
if __name__ == '__main__':
    ...
    modules = [func.replace('create_model_', '') for func in dir(
        sys.modules[__name__]) if 'create_model_' in func]

    assert sys.argv[1] in modules, \
        'Model name must be one of the following: %s' % ', '.join(modules)
